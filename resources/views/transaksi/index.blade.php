        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
                <div class="px-6 pt-6 mb-5 md:w-1/2 2xl:w-1/3">
                    <div>
                        <x-create-button-transaksi href="{{route('transaksi.create') }}"/>
                    </div>
                    <div>
                            @if (session('success'))
                            ...
                            @forelse ($transaksi as $transaksis)
                                <tr class="odd:bg-white odd:dark:bg-gray-800 even:bg-gray-50 even:dark:bg-gray-700">
                                    ...
                                    <td
                                        ...
                                        <p>{{ $transaksis->nama_customer }}</p>
                                    </td>
                                    <td class="px-6 py-4 font-medium text-gray-900 md:whitespace-nowrap dark:text-white">
                                        <p>{{ $transaksis->jumlah_item }}</p>
                                    </td>
                                    <td class="px-6 py-4 font-medium text-gray-900 md:whitespace-nowrap dark:text-white">
                                        <p>{{ $transaksis->total_item}}</p>
                                    </td>
                                
                                    <td class="px-6 py-4">
                                        <div class="flex space-x-3">
                                        <form action="{{ route('transaksi.edit', $transaksis) }}" method="Post">
                                                @csrf
                                                @method('get')
                                                <button type="submit"
                                                    class="text-blue-600 dark:text-red-400 whitespace-nowrap">
                                                    Edit
                                                </button>
                                            </form>

                                            <form action="{{ route('transaksi.destroy', $transaksis) }}" method="Post">
                                                @csrf
                                                @method('delete')
                                                <button type="submit"
