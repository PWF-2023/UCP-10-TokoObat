        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="post" action="{{ route('transaksi.store') }}" class="">
                        @csrf
                        @method('post')
                        <div class="mb-6">
                            <x-input-label for="nama_customer" :value="__('Nama')" />
                            <x-text-input id="nama_customer" name="nama_customer" type="text" class="block w-full mt-1"
                                required autofocus autocomplete="nama_customer" />
                            <x-input-error class="mt-2" :messages="$errors->get('nama_customer')" />
                        </div>
                        <div class="mb-6">
                            <x-input-label for="jumlah_item" :value="__('Jumlah')" />
                            <x-text-input id="jumlah_item" name="jumlah_item" type="number" class="block w-full mt-1"
                                required autofocus autocomplete="jumlah_item" />
                            <x-input-error class="mt-2" :messages="$errors->get('jumlah_item')" />
                        </div>
                        <div class="mb-6">
                            <x-input-label for="total_item" :value="__('Total')" />
                            <x-text-input id="total_item" name="total_item" type="decimal" class="mt-1 block w-full form-control"
                                required autofocus autocomplete="total_item" />
